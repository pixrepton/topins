(function (window) {
  ...
  function updateContainerVisibility(containerName, visible) {
    ...
    if (!config) {
      console.warn(`[render] Brak konfiguracji dla kontenera: ${containerName}`);
      return;
    }
    const container = document.querySelector(config.selector);
    if (!container) {
      console.warn(`[render] Nie znaleziono kontenera: ${config.selector}`);
      return;
    }

    // Debug log dla kontenerów izolacji
    if (
      containerName === 'simplifiedInsulationMode' ||
      containerName === 'detailedInsulationMode'
    ) {
      const state = formEngine.state ? formEngine.state.getAllValues() : {};
      const computedDisplay = container ? window.getComputedStyle(container).display : 'N/A';
      console.log(
        `[render] Kontener ${containerName} (${config.selector}): ${
        ...
        }`,
        {
          ...
          hasHiddenClass: container ? container.classList.contains('hidden') : false,
