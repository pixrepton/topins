  ...
  const TopInstalCalculator = {
    ...
    fillFormFromURLParams() {
      try {
        ...
        setTimeout(() => {
          ...
          // setupDynamicFields USUNIĘTE - formEngine.refresh obsługuje to automatycznie
        }, 100);
      } catch (error) {
    ...
    copyConfigURL() {
      try {
        ...
        if (url === window.location.href) {
          ErrorHandler.showToast('Najpierw wypełnij formularz, aby wygenerować link', 'warning');
          return;
        }
        ...
        ErrorHandler.showToast('Wystąpił błąd podczas kopiowania linku', 'error');
      }
    },
    ...
    showCopySuccessMessage() {
      if (typeof window.showNotification === 'function') {
        ...
        ErrorHandler.showToast('Link do konfiguracji został skopiowany', 'success');
      }
    },
